...
- `CODE-010`: Do not write a comment unless the code contains the
  ...
  the one-line preference and three-line hard limit in `STYLE-032`.
...
- `CODE-021`: A call across a layer, module, process, storage, provider, or
  public boundary MUST use an explicit typed request, response, error, and state
  model appropriate to that boundary.
- `CODE-022`: Raw maps, `any`, provider SDK objects, database records, generated
  transport objects, and framework objects MUST NOT cross into another layer.
- `CODE-023`: Constants, enumerations, contracts, domain models, errors,
  repositories, use cases, adapters, handlers, and validators MUST remain in
  their owning layer and responsibility.
- `CODE-024`: A contract change MUST be evaluated for validation,
  compatibility, migration, failure mapping, and tests before implementation.

## Configuration

- `CODE-025`: Configuration MUST be typed, validated before dependent
  components start, and immutable after composition unless an explicit runtime
  policy contract says otherwise.
- `CODE-026`: Configuration sources, precedence, unknown-key behavior, and safe
  defaults MUST be explicit and tested.
- `CODE-027`: Invalid user or deployment configuration MUST return a normal
  actionable startup error. Panic is limited to impossible programmer-created
  composition states.
- `CODE-028`: Domain MUST NOT read configuration. Application receives typed
  values or policy facts through construction and use-case input.
- `CODE-029`: Secrets MUST be represented by credential references or secret
  ports, not ordinary configuration values.
- `CODE-030`: Diagnostics MAY expose effective non-secret configuration through
  an allowlist and mandatory redaction.

## Error contract

- `CODE-031`: Boundary failures MUST map from a transport-neutral error
...
- `CODE-032`: Adapters MUST map that error into native CLI, MCP, IPC, or HTTP
  outcomes without exposing provider details.

## Documentation

- `CODE-033`: A public external contract documents purpose, ownership,
  ...
  self-explanatory.
- `CODE-034`: Documentation MUST change with public behavior, configuration,
  operational behavior, compatibility, or architecture.
- `CODE-035`: Generated code and generated contracts MUST be reproducible and
