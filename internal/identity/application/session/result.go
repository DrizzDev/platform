...
// Result is the trusted context of a renewed session, or a code-only failure. It
// never carries a token, provider response, or raw cause.
type Result struct {
	failure  *fault.Value
...
func (result Result) Session() handle.Session {
