...
// Append writes one entry to the journal as a single immediate transaction, so the record and its synchronization
// state commit atomically (REL-009).
func (store Store) Append(scope context.Context, entry journal.Entry) error {
	return store.transact(scope, task{name: "append", work: func(scope context.Context, transaction *sql.Tx) error {
		...
		_, failure := transaction.ExecContext(scope,
			"INSERT INTO journal (trace, span, parent, sequence, mark, origin, fidelity, category, payload, state, stamped) "+
				"VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
			...
			entry.Payload(), entry.State().String(), time.Now().Unix())
		return failure
	}})
}
