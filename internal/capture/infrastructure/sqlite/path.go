...
func (location Location) screen(path string) error {
	...
	if info.Mode()&os.ModeSymlink != 0 {
		return errors.New("capture data directory must not be a symlink")
	}

	return nil
}

// inspect rejects a pre-existing database file that is a symlink or is otherwise not a regular file, so a planted node
...
func (location Location) inspect(path string) error {
	...
	case info.Mode()&os.ModeSymlink != 0:
		return errors.New("capture database file must not be a symlink")
	case !info.Mode().IsRegular():
		return errors.New("capture database file must be a regular file")
	}
	return nil
}
