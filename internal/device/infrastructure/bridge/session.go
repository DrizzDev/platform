...
// session is one live sidecar conversation, created per process instance and torn down as a unit on death.
type session struct {
	input io.Writer
...
// scan reads one newline-delimited frame, bounded so a hostile or corrupt frame cannot exhaust memory.
